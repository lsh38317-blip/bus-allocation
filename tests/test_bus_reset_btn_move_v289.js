...
console.log('▶ 테스트2: "배정대수불러오기" 버튼 바로 다음에 위치(DOM 순서상 인접)');
const generateBtn = Array.from(leaderPanel.querySelectorAll('button')).find((b) => b.textContent.indexOf('배정대수불러오기') !== -1);
assert(!!generateBtn, '배정대수불러오기 버튼을 찾음');
assert(generateBtn.nextElementSibling === btn, '배정대수불러오기 버튼의 바로 다음 형제 요소가 배정초기화 버튼임');
...
assert(btn.textContent.indexOf('참석배정 초기화') !== -1, '기본 텍스트(참석배정 초기화) 유지됨(런타임에 참석/귀가로 토글되는 로직은 그대로)');
