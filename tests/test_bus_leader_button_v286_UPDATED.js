...
async function run() {
  ...
  // [v322] 버튼 라벨이 "배정대수불러오기" → "배정유형별 대수 불러오기"로 변경됨(전체배정대수불러오기 버튼 신설에 따른 명확화)
  assert(listEl.textContent.indexOf('배정유형별 대수 불러오기') !== -1, '"배정유형별 대수 불러오기" 버튼 클릭 안내가 표시됨(실제: ' + listEl.textContent.slice(0,80) + ')');
