...
async function run() {
  ...
  const rosterRows = [
    ...
    { 'NO':'2','접수ID':'R2','성명':'김철수','교구':'3교구','구역':'31구역','참석교통수단':'버스','참석배정유형':'본대' },
  ];
  // 시트에 이미 "3교구 31구역" 팀이 다른 배정유형(직장조)으로 저장되어 있는 상황을 재현
  let existingSheetRows = [
    { 'NO':'1','년도':'2026','수양회종류':'하계','행사명':'1차','배정유형':'직장조','교구':'3교구','구역/팀명':'31구역','인원수':'5','배정상태':'미배정','버스':'','분산배정':'','비고':'' },
  ];
  ...
  assert(row && row[4] === '본대', '저장된 행의 배정유형이 "본대"로 기록됨 (실제: ' + (row && row[4]) + ')');
